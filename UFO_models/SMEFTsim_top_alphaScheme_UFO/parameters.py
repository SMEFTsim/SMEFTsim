...
dg1 = Parameter(name = 'dg1',
                nature = 'internal',
                type = 'real',
                value = '(sth**2*(dGf + dMZ2 + (2*cHWB*cth**3*vevhat**2)/(LambdaSMEFT**2*sth)))/(2.*(1 - 2*sth**2))',
                texname = '\\text{dg1}')

dgw = Parameter(name = 'dgw',
                nature = 'internal',
                type = 'real',
                value = '-(cth**2*(dGf + dMZ2 + (2*cHWB*sth**3*vevhat**2)/(cth*LambdaSMEFT**2)))/(2.*(1 - 2*sth**2))',
                texname = '\\text{dgw}')
...
dMW = Parameter(name = 'dMW',
                nature = 'internal',
                type = 'real',
                value = '(dGf/2. + dgw)*MWsm',
                texname = '\\text{dMW}')

dWT = Parameter(name = 'dWT',
                nature = 'internal',
                type = 'real',
                value = 'WT*((dMW*(-(1/(MB + MT - MWsm)) - 4/MWsm + 1/(MB - MT + MWsm) + 1/(-MB + MT + MWsm) + 1/(MB + MT + MWsm) + (4*MWsm*(MB**2 + MT**2 - 4*MWsm**2))/((MB**2 - MT**2)**2 + (MB**2 + MT**2)*MWsm**2 - 2*MWsm**4)))/2. + 2*(dgw + (vevhat*(ee*(3*cHtbRe*MB*MT*MWsm**2 + cHQ3*((MB**2 - MT**2)**2 + (MB**2 + MT**2)*MWsm**2 - 2*MWsm**4))*vevhat + 6*MWsm**2*(ctWRe*MT*(MB**2 - MT**2 + MWsm**2) + cbWRe*MB*(-MB**2 + MT**2 + MWsm**2))*sth*cmath.sqrt(2)))/(ee*LambdaSMEFT**2*((MB**2 - MT**2)**2 + (MB**2 + MT**2)*MWsm**2 - 2*MWsm**4))))',
                texname = '\\text{dWT}')

dWW = Parameter(name = 'dWW',
                nature = 'internal',
                type = 'real',
                value = '(dMW*WW)/MWsm + (2*dgw + (2*(6*cHj3 + cHl311 + cHl322 + cHl333)*vevhat**2)/(9.*LambdaSMEFT**2))*WW',
                texname = '\\text{dWW}')

MW = Parameter(name = 'MW',
               nature = 'internal',
               type = 'real',
               value = 'dMW*(1 - linearPropCorrections) + MWsm',
               texname = 'M_W')
